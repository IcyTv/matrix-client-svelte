<script lang="ts">
	...
	const historyHasLastPage = window.history.length > 1;
</script>

{#key $page.routeId}
	<div in:scale={{ duration: 200 }} out:scale={{ duration: 200 }} class="flex h-full w-full flex-row text-white">
		<div class="flex h-full w-[20%] flex-row justify-end bg-slate-700 p-4">
			<LeftPane bind:selected />
		</div>
		<div class="h-full w-[80%] bg-slate-600">
			<RightPane {selected} />
			<div class="absolute top-20 right-12 flex flex-col items-center">
				<CloseOutline class="h-8 w-8 text-slate-300" />
