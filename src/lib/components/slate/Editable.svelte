<script lang="ts" context="module">
	import { Editor, Node as SlateNode, Element as SlateElement, Path, Range, Transforms } from 'slate';
	import { afterUpdate } from 'svelte';
	import { onMount, tick } from 'svelte';
	import { debounce } from '@aicacia/debounce';
	import scrollIntoView from 'scroll-into-view-if-needed';
	import { direction } from 'svelte-slate/direction';
	import Children from 'svelte-slate/components/Children.svelte';
	import DefaultElement from 'svelte-slate/components/DefaultElement.svelte';
	import DefaultLeaf from 'svelte-slate/components/DefaultLeaf.svelte';
	import DefaultPlaceholder from 'svelte-slate/components/DefaultPlaceholder.svelte';
	import { getEditor, getEventsContext, getValueContext, handleEvent } from 'svelte-slate';
	import { defaultDecorate, getDecorateContext, getEditorContext, getFocusedContext, getReadOnlyContext, getSelectionContext } from 'svelte-slate';
	import { HAS_BEFORE_INPUT_SUPPORT, IS_CHROME, IS_FIREFOX, IS_FIREFOX_LEGACY, IS_IOS, IS_QQBROWSER, IS_SAFARI } from 'svelte-slate/environment';
	import hotkeys from 'svelte-slate/hotkeys';
	import { DOMNode, DOMRange, getDefaultView, isDOMElement, isPlainTextOnlyPaste } from 'svelte-slate/dom';
	import { isDOMNode } from 'svelte-slate/dom';
	import {
		createContext,
		createContextKey,
	...
	} from 'svelte-slate/utils';
	import type { ISvelteEditor } from 'svelte-slate/withSvelte';
	import { EDITOR_TO_ELEMENT, EDITOR_TO_WINDOW, ELEMENT_TO_NODE, IS_FOCUSED, IS_READ_ONLY, NODE_TO_ELEMENT, PLACEHOLDER_SYMBOL } from 'svelte-slate/weakMaps';
	import type { IElementProps } from 'svelte-slate/components/InternalElement.svelte';
	import type { ILeafProps, IPlaceholderProps } from 'svelte-slate/components/InternalLeaf.svelte';
	import { getContainerContext } from 'svelte-slate';
	...
	export function hasEditableTarget(editor: ISvelteEditor, target: EventTarget | null): target is DOMNode {
		return isDOMNode(target) && hasDOMNode(editor, target, { editable: true });
	}

	export function isTargetInsideNonReadonlyVoid(editor: ISvelteEditor, target: EventTarget | null): boolean {
...
<script lang="ts">
	...
	$: if ($editorContext.children.length === 1 && Array.from(SlateNode.texts($editorContext)).length === 1 && SlateNode.string($editorContext) === '' && !state.isComposing) {
	...
	function onDOMSelectionChange() {
		if (state.isComposing || state.isDraggingInternally || state.updateSelectionTimeoutId) {
			...
			const anchorNodeSelectable = hasEditableTarget(editor, domSelection.anchorNode) || isTargetInsideNonReadonlyVoid(editor, domSelection.anchorNode);

			const focusNodeSelectable = hasEditableTarget(editor, domSelection.focusNode) || isTargetInsideNonReadonlyVoid(editor, domSelection.focusNode);
			...
			if (hotkeys.isRedo(event)) {
				...
				if (IS_CHROME || IS_SAFARI) {
					if (editor.selection && (hotkeys.isDeleteBackward(event) || hotkeys.isDeleteForward(event)) && Range.isCollapsed(editor.selection)) {
						...
						if (SlateElement.isElement(currentNode) && Editor.isVoid(editor, currentNode) && Editor.isInline(editor, currentNode)) {
							event.preventDefault();
							Editor.deleteBackward(editor, { unit: 'block' });
							return;
						}
	...
	function onFocus(event: FocusEvent) {
		if (!state.readOnly && !state.updateSelectionTimeoutId && hasEditableTarget(editor, event.target)) {
			const el = toDOMNode(editor, editor);
